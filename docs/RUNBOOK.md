...
収集と記事生成は **1日2回**。1回につき記事2本、1日あわせて **4本** が目標。

| JST | 動くもの | 内容 |
|---|---|---|
| 09:30 | `collect.yml` | 収集。**`--dry-run` で走る**（後述） |
| 10:00 | `daily-article.yml` | 記事2本 → ビルド → 公開 |
| 22:30 | `collect.yml` | 収集。seen.json を更新 |
| 23:00 | `daily-article.yml` | 記事2本 → ビルド → 公開 |

朝の収集だけ `--dry-run` を付けている。**これは「実行しない」という意味ではない** —— `collect.py --dry-run` は `seen.json` を更新しないだけで、ダイジェストは通常どおり書き出す。

理由: collect.py は上位30件ではなく **取得できた新規アイテムを全部** `seen.json` に記録する。朝に記録してしまうと、その日に拾えた候補がまとめて既読になり、夜の生成に回せる候補が激減する。朝に消費するのは2本ぶんだけなので、残りは夜にもう一度出せる状態で置いておきたい。記録は夜の実行にまとめて任せる。

記事生成が「今日の記事が既にあるから終了」と判断しないよう、**本数ベース**の判定にしてある（4本以上なら終了、未満なら2本書く）。1日2回走る以上、朝の2本が既にあるのが正常な状態のため。
...
定期実行（10:00 と 23:00）が、ダイジェストより先に**ここを上から消化する**。1回につき最大2本。
...
この場合は**指定した URL だけ**が記事になる。2本に水増ししない。

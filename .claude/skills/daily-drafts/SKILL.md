...
- 元記事に無い数字を書かない
- 円換算を書かない
- 画像をダウンロードして `public/` に置かない。**公式サイト（メーカー公式ページ／クラファンのプロジェクトページ）の画像は `thumbnail` にURLを書いて引用してよい。** その場合 `thumbnail_credit` に出典を必ず書く。他のニュースサイトの画像は使わない
- **「日本から見るとどうか」は必須。** 技適・電波法・国内流通・日本語対応を具体的に
- クラファン案件は「まだ製品ではない」「調達額は取得時点」「配送時期は予定」を明記

画像の優先順位は次のとおり。**上から順に試して、取れたところで止める。**

1. YouTube の実機動画（`embeds`）— サムネイルが自動でカード画像になる
2. **公式サイトの製品画像**（`thumbnail` + `thumbnail_credit`）— メーカー公式・クラファンページのみ
3. 何も無ければ省略（自動生成のアイキャッチが出る）
...
条件の詳細は `docs/PLAYBOOK.md` の「3.5 画像をどう用意するか」を読むこと。
**画像URLは必ず WebFetch で存在を確認してから書く。** 404 のURLを書くと自動生成画像に落ちるだけで無駄になる。
...
検証（5と6）が両方通っていることを確認してから、コミットして push する。

```bash
git add content/posts/ data/
git commit -m "記事3本追加 <今日の日付>"
git pull --rebase
git push
```

push すると `deploy.yml` が走り、数分で https://gadgetterminal.com/ に反映される。
**検証が通っていない記事は push しない。** 直してから出す。
...
> 修正後にもう一度 push すれば、数分でサイトに反映されます。

## やらないこと

- **検証を通していない記事を push しない。** 公開は自動だが、YAML とビルドの確認は必須
